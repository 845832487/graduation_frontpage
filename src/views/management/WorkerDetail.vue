  ...
  <div style="margin: 0px">
    ...
    <div class="demo-pagination-block">
      ...
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="saveAddWorker"
        >确认</el-button>
      </span>
        </template>
      </el-dialog>
      ...
      <el-dialog
        ...
        <el-form model="form" label-width="120px">
          ...
          <el-table :data="workerList" stripe style="width: 100%;padding: 15px">
            <el-table-column prop="id" label="账号" width="180"/>
            <el-table-column prop="name" label="姓名" width="180"/>
            <el-table-column prop="sex" label="性别" width="180" :formatter="sexFormat"/>
            <el-table-column prop="job" label="职位" width="180"/>
...
export default {
  ...
  methods: {
    delMultiWorker() {
      request.delete("/workerDetail/delMultiWorkers", {
        data: {
          workerDetails: this.selectedList
        }
      }).then(res => {
        if (res.code === 666) {
          this.load();
    ...
    upLoad() {
      request.put("/workerDetail/addMultiWorkers", this.workerList).then(res => {
        if (res.code === 666) {
          this.load();
          ElMessage.success(res.data);
        } else ElMessage.error(res.msg);
    ...
    delInfo(row) {
      ...
      request.delete("/workerDetail/delSingleWorker", {
        params: {
    ...
    saveAddWorker() {
      if (this.preId === this.form.id) {
        //修改
        request.put("/workerDetail/updWorker", this.form).then(res => {
          if (res.code === 666) {
        ...
        //添加
        request.post("/workerDetail/addOneWorker", this.form).then(res => {
          if (res.code === 666) {
